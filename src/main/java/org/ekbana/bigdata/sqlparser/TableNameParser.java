...
public final class TableNameParser {
    ...
    private String[] filteredQueryAsTokens = null;
    ...
    public TableNameParser(final String sql) {
        ...
        String[] tok = cleansed.split(REGEX_SPACE);

        String[] toke = normalizeOperators(tok);
        this.filteredQueryAsTokens = toke;

        String[] tokens = removeAsFromQuery(toke);
    ...
    private String[] normalizeOperators(String[] tok) {
        String str = "";
        for (String s : tok) {
            if (s.contains("<=")) {
                str=str+" "+s.replace("<="," <= ");
        ...
        return str.split(REGEX_SPACE);
    }

        private String[] removeAsFromQuery (String[]tokens){
            String tok = "";
            int j = 0;
            while (j < tokens.length) {
                if (tokens[j].equals("as") || tokens[j].equals("AS")) {
                    j += 2;
                } else {
                    tok = tok + " " + tokens[j];
                    j += 1;
        ...
        public String[] getfilteredQueryAsTokens () {
            return this.filteredQueryAsTokens;
        }
    }
